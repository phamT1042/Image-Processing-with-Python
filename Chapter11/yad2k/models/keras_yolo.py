...
class YoloHead(tf.keras.layers.Layer):
    ...
    def call(self, feats):
        ...
        anchors_tensor = tf.reshape(tf.constant(self.anchors, dtype=tf.float32), [1, 1, 1, num_anchors, 2])
        
        shape = tf.shape(feats)
        batch_size, height, width = shape[0], shape[1], shape[2]
        
        feats = tf.reshape(feats, [batch_size, height, width, num_anchors, self.num_classes + 5])
        ...
        grid_x = tf.range(width, dtype=tf.float32)
        grid_y = tf.range(height, dtype=tf.float32)
        grid_x, grid_y = tf.meshgrid(grid_x, grid_y)
        grid_x = tf.reshape(grid_x, [1, height, width, 1])
        grid_y = tf.reshape(grid_y, [1, height, width, 1])
        
        box_xy = (box_xy + tf.stack([grid_x, grid_y], axis=-1)) / tf.cast(tf.stack([width, height]), tf.float32)
        box_wh = box_wh * anchors_tensor / tf.cast(tf.stack([width, height]), tf.float32)
...
def yolo_boxes_to_corners(box_xy, box_wh):
    ...
    box_xy = tf.convert_to_tensor(box_xy) 
    box_wh = tf.convert_to_tensor(box_wh)
...
def yolo_eval(yolo_outputs, image_shape, num_classes, max_boxes=10, score_threshold=.6, iou_threshold=.5):
    ...
    box_confidence = tf.convert_to_tensor(box_confidence)
    box_class_probs = tf.convert_to_tensor(box_class_probs)
    
    # Filter boxes based on scores
    scores = box_confidence * box_class_probs
    boxes = tf.reshape(boxes, [-1, 4])
    scores = tf.reshape(scores, [-1, num_classes])  # Thay đổi class_names thành num_classes
